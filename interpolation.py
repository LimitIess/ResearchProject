...
def read_data_and_interpolate():
    """
    Read data from excel file and interpolate <15 values in bcov19Kom files.
    """
    df = read_excel()
    ...
    for folder in list_of_folder:
        ...
        #df_2.update(tmp_new_df, overwrite=False)
    ...
    #tmp_new_df.to_csv("testing_interpolation.csv")
    #print(tmp_new_df)
    #df = pd.read_csv("Testing/bcov19Kom.csv", sep=',', encoding='utf-8')
    #df.replace('..', np.nan, inplace=True)
    #df = df.combine_first(tmp_new_df)
    #print(df)
    
def read_excel():
    """Read excel file"""
    #read excel file
    df_excel = pd.read_excel("excel/2022/Folkhalsomyndigheten_Covid19_Apr 01 2022.xlsx", sheet_name=None)
    return df_excel

def average_interpolation_total_bcov19Kom(df:pd.DataFrame):
    """
    ...
    na_index_list = list(df.loc[pandas.isna(df["tot_antal_fall"]), :].index)
    ...
    for value in range(min(na_index_list), max(na_index_list)+1):
        ...
        #If there are more than 1 case
            ...
            #Set the tmp value holder to the difference between the value and the tmp value holder
                ...
                df.iat[value,9] = diff
                #print(f"Larger difference: {tmp}")
        #print(f"Value for iteration: {df.iat[value,9]}")
